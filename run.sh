...
function usage() {
  ...
  echo "./run.sh config/test.yaml"
  echo "./run.sh config/test.yaml --debug"
  echo "./run.sh config/test.yaml --debug=5005"
  echo "./run.sh --debug=5005 config/test.yaml"
  exit 0
}
...
if [[ -n $CFG ]] ; then
    ...
    CFG="target/config/$ENV_OVERRIDE.yaml"
    ...
    if [[ ! -z "${USER-}" ]] ; then
        USER_CFG="target/config/$USER.yaml"         # Ex: vb.yaml
    else
        USER_CFG=""
    fi
    if [[ -e "$LOCATION_FILE" ]]; then
      LOCATION=$(echo $(cat "$LOCATION_FILE") | tr '[:upper:]' '[:lower:]')
    else
      LOCATION=$DESKTOP_LOCATION
    fi
    LOCATION_CFG="target/config/$LOCATION.yaml" # Ex: desktop.conf, prod.conf
...
BASE_ENV="base.env"
info "Looking for environment configs under $(pwd): (location: $LOCATION_ENV), (base: $BASE_ENV)"
...
# Work-around for issues with different shell versions as 'source' uses PATH to resolve files.
# The sourced file sources others, hence overriding PATH instead of just specifying ./ prefix.
# Ensuring local folder is first in path so 'source' uses the local folder.
# Other option is to unset shopts for sourcepath
OLDPATH=$PATH
PATH=.:$PATH
source $ENV_FILE
PATH=$OLDPATH
